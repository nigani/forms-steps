...
export default function WidgetBox(props) {
  ...
  const onAddValue = (newRecord) =>
    setState((prevState) => {
      ...
      const record = prevState.records.find(
        (o) => o.dateDist === newRecord.dateDist
      );
      if (record)
        recordMod = {
      ...
      if (recordMod.dist === 0) {
        return {
          ...prevState,
          records: prevState.records.filter(
            (o) => o.dateDist !== newRecord.dateDist
          ),
        };
      } else {
        return {
          ...prevState,
          records: [
            ...prevState.records.filter(
              (o) => o.dateDist !== newRecord.dateDist
            ),
            recordMod,
          ].sort((a, b) =>
            a.dateDist > b.dateDist ? 1 : a.dateDist < b.dateDist ? -1 : 0
          ),
        };
      }
    });

  const onRemoveValue = (newRecord) =>
    setState((prevState) => ({
      ...prevState,
      records: prevState.records.filter(
        (o) => o.dateDist !== newRecord.dateDist
